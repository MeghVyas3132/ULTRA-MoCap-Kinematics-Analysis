...
class EMGLSTMMSAModel(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(d_model + out_dim, d_model + out_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(d_model + out_dim, num_classes),
        )
    ...
    def forward(self, emg_x):
        ...
        fused = torch.cat([input_summary, output_summary], dim=-1)
...
SMOKE_TEST_SUBJECT = os.getenv("SMOKE_TEST_SUBJECT")
...
results_folder = "Code-base/MocapDatasetScripting_REALLAB/results/Results_ConvBiGRU"
...
def summarize_emg_variant_vs_baseline(results_dir, current_emg_csv, baseline_emg_csv):
